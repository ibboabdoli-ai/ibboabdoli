...
`archive/public_html/` preserves the historic site for reference, not deployment. Historical release markers live in `docs/history/`. The three old automatic HTML/social-image patch workflows are preserved, inactive, in `archive/github-workflows/`. Do not edit these archived locations to update production or move the legacy workflows back into `.github/workflows/` without review.
...
Open http://127.0.0.1:4173. Only explicitly allowlisted files are copied to `dist/`; archives, tooling and documentation cannot leak into the production build. The local server applies the production security headers. Vercel builds previews for Git-integrated branches and production after a merge to `main`.

## Quality checks

`tests/static_checks.py` checks internal links, fragments, metadata, hreflang, sitemap, JSON-LD, inline content restrictions and PDF signatures. The browser suite checks responsive layouts, keyboard navigation, no-JavaScript content, motion preferences and mocked form success/error handling. Formspree is mocked in automated tests: a passing test does not prove inbox delivery. CI also records Lighthouse lab measurements; these are not field Core Web Vitals.

The permanent `Portfolio quality` workflow has read-only repository permissions and cannot modify source. It runs on pull requests and main pushes. Lab budgets are CLS <= 0.1, performance >= 85, accessibility >= 95 and SEO >= 95. After a main push, a separate production job waits for Vercel, compares public page and asset bytes with this build, checks headers, the apex redirect and excluded paths, then measures the public production URL with Lighthouse. Reports are retained as GitHub Actions artifacts for seven days. No contact form is submitted by the live checker.
